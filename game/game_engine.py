...
class GameEngine:
    ...
    def get_opponent_id(self, player_id):
        for player_id_in_order in self.player_order:
    ...
    def start_game(self):
        first = random.choice(self.player_order)
    ...
    def _require_active_player(self, player_id):
        if self.is_over:
            raise ValueError(
    ...
    def perform_attack(
        ...
        attacker_player = self.players[player_id]

        defender_id = self.get_opponent_id(player_id)

        defender_player = self.players[defender_id]

        if not 0 <= attacker_slot_index < len(
        ...
        if not 0 <= target_slot_index < len(
        ...
        attacker = attacker_player.active_slots[
            attacker_slot_index
        ]

        target = defender_player.active_slots[
            target_slot_index
        ]
        ...
        if use_super and not attacker.super_unlocked:
            raise ValueError(
                "That card's super isn't unlocked yet."
            )
        ...
        result = self.abilities.resolve(
            ability_id,
            {
                ...
                "all_allies": [
                    card
                    for card in attacker_player.active_slots
                    if card is not None
                ],
                "all_enemies": [
                    card
                    for card in defender_player.active_slots
                    if card is not None
                ],
            }
        )
        ...
        self._check_for_defeats(attacker_player)
        self._check_for_defeats(defender_player)
    ...
    def perform_swap(
        ...
        self._emit({
            ...
            "activeSlotIndex": active_slot_index,
        })
    ...
    def _check_for_defeats(self, player):
        cards_to_check = (
    ...
    def _check_win_condition(self):
        for player_id in self.player_order:
            if self.players[player_id].has_lost:
                self.is_over = True

                self.winner_id = self.get_opponent_id(
                    player_id
                )
    ...
    def _advance_turn(self):
        self.active_player_id = self.get_opponent_id(
            self.active_player_id
        )

        self.turn_number += 1

    def get_state_for(self, player_id):
        ...
        return {
            ...
            "opponent": {
                ...
                "active": [
                    ...
                    for card in opponent.active_slots
                ],
